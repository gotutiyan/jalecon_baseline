...
def complexity_wise_MAE(ref: List[float], hyp: List[float]) -> dict:
    results = {
...
def evaluation(
    ...
    with torch.no_grad():
        for batch in valid_loader:
            ...
            # Output may be outside the range [0, 1] because the sigmoid function is not used.
            hyp += torch.clip(outputs.logits, min=0, max=1).view(-1).cpu().tolist()
    r2 = R2(ref, hyp)
...
## If you want to check the loss on the validation set, this function will help you.
# def valid(model,
#     loader: DataLoader,
#     epoch: int,
#     accelerator: Accelerator
...
def main(args):
    ...
    if args.restore_dir is not None:
        ...
        model = BertForSequenceClassification.from_pretrained(
            ...
            num_labels=1 # num_labels=1 is to solve as a regression task
        )
    ...
    cv_split = cv_split_data[args.cv_split-1]
    ...
    for epoch in range(current_epoch, args.epochs):
        ...
        accelerator.wait_for_everyone()
        if accelerator.is_main_process:
            # Save a checkpoint as the last checkpoint for each epoch
            accelerator.unwrap_model(model).save_pretrained(args.outdir)
            state_dict = {
                'current_epoch': epoch,
...
def get_parser():
    ...
    parser.add_argument('--cv_split', type=int, choices=[1,2,3,4,5], default=1)
    parser.add_argument(
        "--lr_scheduler_type",
        default="linear",
